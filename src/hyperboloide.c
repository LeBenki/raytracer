...
** Last update Sun May 28 19:43:47 2017 Melvin Nalepka
*/
...
float		intersect_hyperboloide(sfVector3f eye,
...
sfVector3f	get_normal_hyperboloide(sfVector3f point, float rad)
{
  sfVector3f	normal;

  normal.x = point.x;
  normal.y = point.y;
  normal.z = tan(-(rad / 180 / M_PI)) * point.z;
  return (normal);
}

float		draw_hyperboloide(t_ray *ray)
{
  float		cos_angle;
  sfVector3f	point;
  sfVector3f	normal;
  ...
  normal.z = tan(-(ray->radius[ray->count] / 180 / M_PI)) * point.z;
