...
**
** Started on  Wed Feb  8 21:07:09 2017 Lucas Benkemoun
** Last update Sun May 28 19:36:16 2017 Melvin Nalepka
*/

#include "line.h"

static float	one_soluce(float a, float b, sfVector3f dir)
{
  float		k1;
...
float		intersect_cylinder(sfVector3f eye,
				   sfVector3f dir, float radius)
{
  ...
  if (delta > 0)
    ...
    return (one_soluce(a, b, dir));
  return (-1.0);
}

sfVector3f	get_normal_cylinder(sfVector3f dir)
{
  sfVector3f    normal;
...
float		draw_cyl(t_ray *ray)
{
  ...
  if (point.z < ray->limit[ray->count].y)
    return ((ray->k[ray->count] = -1));
  normal = get_normal_cylinder(point);
  normal = rotate_zyx(normal, set_vectorff(ray->rot[ray->count].x,
   					   ray->rot[ray->count].y,
   					   ray->rot[ray->count].z));
